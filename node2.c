...
typedef void node_itr_t(rb_ast2_t *ast, void *ctx, NODE *node);
static void iterate_node_values(rb_ast2_t *ast, node_buffer_list_t *nb, node_itr_t * func, void *ctx);
...
static void
node_buffer_list_free(rb_ast2_t *ast, node_buffer_list_t * nb)
{
...
static void
parser_string_free(rb_ast2_t *ast, rb_parser_string_t *str)
{
    if (!str) return;
    xfree(str->ptr);
    xfree(str);
...
static void
parser_ast_token_free(rb_ast2_t *ast, rb_parser_ast_token_t *token)
{
    if (!token) return;
...
static void
parser_tokens_free(rb_ast2_t *ast, rb_parser_ary_t *tokens)
{
...
static void
parser_nodes_free(rb_ast2_t *ast, rb_parser_ary_t *nodes)
{
...
static void
free_ast_value(rb_ast2_t *ast, void *ctx, NODE *node)
{
    switch (nd_type(node)) {
      case NODE_STR:
...
static void
rb_node_buffer_free(rb_ast2_t *ast, node_buffer_t *nb)
{
    if (nb->tokens) {
...
static NODE *
ast_newnode_in_bucket(rb_ast2_t *ast, node_buffer_list_t *nb, size_t size, size_t alignment)
{
    size_t padding;
    NODE *ptr;
...
NODE *
rb_ast2_newnode(rb_ast2_t *ast, enum node_type type, size_t size, size_t alignment)
{
...
rb_ast_id_table_t *
rb_ast2_new_local_table(rb_ast2_t *ast, int size)
{
...
rb_ast_id_table_t *
rb_ast2_resize_latest_local_table(rb_ast2_t *ast, int size)
{
...
void
rb_ast2_delete_node(rb_ast2_t *ast, NODE *n)
{
    (void)ast;
    (void)n;
...
#ifdef UNIVERSAL_PARSER
rb_ast2_t *
rb_ast2_new(const rb_parser_config_t *config)
{
    node_buffer_t *nb = rb_node_buffer_new(config);
    rb_ast2_t *ast = (rb_ast_t *)config->calloc(1, sizeof(rb_ast2_t));
    ast->config = config;
    ast->node_buffer = nb;
    return ast;
}
...
rb_ast2_new(void)
{
    ...
    rb_ast2_t *ast = ruby_xcalloc(1, sizeof(rb_ast2_t));
    ast->node_buffer = nb;
    return ast;
}
#endif
...
static void
iterate_buffer_elements(rb_ast2_t *ast, node_buffer_elem_t *nbe, long len, node_itr_t *func, void *ctx)
{
    long cursor;
...
static void
iterate_node_values(rb_ast2_t *ast, node_buffer_list_t *nb, node_itr_t * func, void *ctx)
{
    node_buffer_elem_t *nbe = nb->head;
...
static void
script_lines_free(rb_ast2_t *ast, rb_parser_ary_t *script_lines)
{
