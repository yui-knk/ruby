...
#define YYFPRINTF(out, ...)	rb_parser2_printf(p, __VA_ARGS__)
#define YY_LOCATION_PRINT(File, loc, p) \
     rb_parser2_printf(p, "%d.%d-%d.%d", \
...
#define RUBY_SET_YYLLOC_FROM_STRTERM_HEREDOC(Current)			\
    rb_parser2_set_location_from_strterm_heredoc(p, &p->lex.strterm->u.heredoc, &(Current))
#define RUBY_SET_YYLLOC_OF_DELAYED_TOKEN(Current)			\
    rb_parser2_set_location_of_delayed_token(p, &(Current))
#define RUBY_SET_YYLLOC_OF_HEREDOC_END(Current)				\
    rb_parser2_set_location_of_heredoc_end(p, &(Current))
#define RUBY_SET_YYLLOC_OF_DUMMY_END(Current)				\
    rb_parser2_set_location_of_dummy_end(p, &(Current))
#define RUBY_SET_YYLLOC_OF_NONE(Current)				\
    rb_parser2_set_location_of_none(p, &(Current))
#define RUBY_SET_YYLLOC(Current)					\
    rb_parser2_set_location(p, &(Current))
...
# define SHOW_BITSTACK(stack, name) (p->debug ? rb_parser2_show_bitstack(p, stack, name, __LINE__) : (void)0)
...
{
    if (p->debug) {
        rb_parser2_printf(p, "after-shift: %+"PRIsVALUE"\n", p->s_value);
    }
...
after_reduce(int len, struct parser_params *p)
{
    for (int i = 0; i < len; i++) {
        ...
        if (p->debug) {
            rb_parser2_printf(p, "after-reduce pop: %+"PRIsVALUE"\n", tos);
        }
    }
    if (p->debug) {
        rb_parser2_printf(p, "after-reduce push: %+"PRIsVALUE"\n", p->s_lvalue);
    }
...
{
    if (p->debug) {
        rb_parser2_printf(p, "after-shift-error-token:\n");
    }
...
after_pop_stack(int len, struct parser_params *p)
{
    for (int i = 0; i < len; i++) {
        VALUE tos = rb_ary_pop(p->s_value_stack);
        if (p->debug) {
            rb_parser2_printf(p, "after-pop-stack pop: %+"PRIsVALUE"\n", tos);
        }
    }
}
#else
...
string_buffer_free(struct parser_params *p)
{
    ...
    while (elem) {
        ...
        for (long i = 0; i < elem->used; i++) {
            rb_parser2_string_free(p, elem->buf[i]);
        }
...
VALUE rb_parser_reg_compile2(struct parser_params* p, VALUE str, int options);
int rb_reg_fragment_setenc2(struct parser_params*, rb_parser_string_t *, int);
enum lex_state_e rb_parser2_trace_lex_state(struct parser_params *, enum lex_state_e, enum lex_state_e, int);
VALUE rb_parser_lex_state_name2(struct parser_params *p, enum lex_state_e state);
void rb_parser2_show_bitstack(struct parser_params *, stack_type, const char *, int);
PRINTF_ARGS(void rb_parser2_fatal(struct parser_params *p, const char *fmt, ...), 2, 3);
YYLTYPE *rb_parser2_set_location_from_strterm_heredoc(struct parser_params *p, rb_strterm_heredoc_t *here, YYLTYPE *yylloc);
YYLTYPE *rb_parser2_set_location_of_delayed_token(struct parser_params *p, YYLTYPE *yylloc);
YYLTYPE *rb_parser2_set_location_of_heredoc_end(struct parser_params *p, YYLTYPE *yylloc);
YYLTYPE *rb_parser2_set_location_of_dummy_end(struct parser_params *p, YYLTYPE *yylloc);
YYLTYPE *rb_parser2_set_location_of_none(struct parser_params *p, YYLTYPE *yylloc);
YYLTYPE *rb_parser2_set_location(struct parser_params *p, YYLTYPE *yylloc);
void ruby_show_error_line2(struct parser_params *p, VALUE errbuf, const YYLTYPE *yylloc, int lineno, rb_parser_string_t *str);
...
#define internal_id rb_parser2_internal_id
...
#define dvar_defined_ref rb_parser2_dvar_defined_ref
...
#define yytnamerr(yyres, yystr) (YYSIZE_T)rb_yytnamerr2(p, yyres, yystr)
RUBY_FUNC_EXPORTED size_t rb_yytnamerr2(struct parser_params *p, char *yyres, const char *yystr);
...
#define debug_token_line(p, name, line) do { \
        if (p->debug) { \
            ...
            rb_parser2_printf(p, name ":%d (%d: %"PRIdPTRDIFF"|%"PRIdPTRDIFF"|%"PRIdPTRDIFF")\n", \
...
#ifndef RIPPER
rb_parser_string_t *
rb_str_to_parser2_string(rb_parser_t *p, VALUE str)
{
    /* Type check */
...
void
rb_parser2_string_free(rb_parser_t *p, rb_parser_string_t *str)
{
    if (!str) return;
...
{
    if (!token) return;
    rb_parser2_string_free(p, token->str);
    xfree(token);
}
...
# define foreach_ary(ptr) \
    ...
    switch (ary->data_type) {
      ...
      case PARSER_ARY_DATA_SCRIPT_LINE:
        foreach_ary(data) {rb_parser2_string_free(p, *data);}
        break;
      case PARSER_ARY_DATA_NODE:
        /* Do nothing because nodes are freed when rb_ast2_t is freed */
        break;
      default:
        rb_bug("unexpected rb_parser_ary_data_type: %d", ary->data_type);
        break;
...
%printer {
    if ((NODE *)$$ == (NODE *)-1) {
        rb_parser2_printf(p, "NODE_SPECIAL");
    }
    else if ($$) {
        rb_parser2_printf(p, "%s", parser_node_name(nd_type(RNODE($$))));
    }
} <node> <node_fcall> <node_args> <node_args_aux> <node_opt_arg>
  <node_kw_arg> <node_block_pass> <node_masgn> <node_def_temp> <node_exits>
%printer {
    rb_parser2_printf(p, "%"PRIsVALUE, rb_id2str($$));
} <id>
%printer {
    switch (nd_type(RNODE($$))) {
      case NODE_INTEGER:
        rb_parser2_printf(p, "%+"PRIsVALUE, rb_node_integer_literal_val($$));
        break;
      case NODE_FLOAT:
        rb_parser2_printf(p, "%+"PRIsVALUE, rb_node_float_literal_val($$));
        break;
      case NODE_RATIONAL:
        rb_parser2_printf(p, "%+"PRIsVALUE, rb_node_rational_literal_val($$));
        break;
      case NODE_IMAGINARY:
        rb_parser2_printf(p, "%+"PRIsVALUE, rb_node_imaginary_literal_val($$));
        break;
      default:
        break;
    }
...
%printer {
    rb_parser2_printf(p, "$%ld", RNODE_NTH_REF($$)->nd_nth);
} tNTH_REF
%printer {
    rb_parser2_printf(p, "$%c", (int)RNODE_BACK_REF($$)->nd_nth);
} tBACK_REF
              ...
              k_end
                {
                    ...
                    rb_ast_id_table_t *tbl = rb_ast2_new_local_table(p->ast, 1);
...
# define set_yylval_node(x) {				\
  YYLTYPE _cur_loc;					\
  rb_parser2_set_location(p, &_cur_loc);			\
  yylval.node = (x);					\
...
parser_append_tokens(struct parser_params *p, rb_parser_string_t *str, enum yytokentype t, int line)
{
    ...
    if (p->debug) {
        ...
        rb_parser2_printf(p, "Append tokens (line: %d) [%d, :%s, \"%s\", [%d, %d, %d, %d]]\n",
        ...
        rb_parser2_string_free(p, str_escaped);
    }
}
...
parser_dispatch_delayed_token(struct parser_params *p, enum yytokentype t, int line)
{
    ...
    if (p->keep_tokens) {
        ...
        rb_parser2_string_free(p, p->delayed.token);
    }
...
parser_show_error_line(struct parser_params *p, const YYLTYPE *yylloc)
{
    ...
    ruby_show_error_line2(p, p->error_buffer, yylloc, lineno, str);
}
...
void
ruby_show_error_line2(struct parser_params *p, VALUE errbuf, const YYLTYPE *yylloc, int lineno, rb_parser_string_t *str)
{
    VALUE mesg;
...
#ifndef RIPPER
    if (p->debug) {
        rb_parser2_printf(p, "vtable_alloc:%d: %p\n", line, (void *)tbl);
    }
#endif
    return tbl;
}
...
#ifndef RIPPER
    if (p->debug) {
        rb_parser2_printf(p, "vtable_free:%d: %s(%p)\n", line, name, (void *)tbl);
    }
#endif
    if (!DVARS_TERMINAL_P(tbl)) {
        if (tbl->tbl) {
...
#ifndef RIPPER
    if (p->debug) {
        rb_parser2_printf(p, "vtable_add:%d: %s(%p), %s\n",
                         line, name, (void *)tbl, rb_id2name(id));
    }
#endif
    if (DVARS_TERMINAL_P(tbl)) {
        rb_parser2_fatal(p, "vtable_add: vtable is not allocated (%p)", (void *)tbl);
        return;
    }
    if (tbl->pos == tbl->capa) {
        tbl->capa = tbl->capa * 2;
...
{
    if (p->debug) {
        rb_parser2_printf(p, "vtable_pop:%d: %s(%p), %d\n",
                         line, name, (void *)tbl, n);
    }
    if (tbl->pos < n) {
        rb_parser2_fatal(p, "vtable_pop: unreachable (%d < %d)", tbl->pos, n);
        return;
    }
    tbl->pos -= n;
}
...
#ifndef RIPPER
rb_ast2_t*
rb_parser_compile2(rb_parser_t *p, rb_parser_lex_gets_func *gets, VALUE fname, rb_parser_input_data input, int line)
{
    p->lex.gets = gets;
    p->lex.input = input;
...
#define SPECIAL_PUNCT(idx) ( \
        ...
        BIT('&', idx) | BIT('`', idx) | BIT('\'', idx) | BIT('+', idx) | \
        BIT('0', idx))
const uint_least32_t ruby_global_name_punct_bits2[] = {
    SPECIAL_PUNCT(0),
    SPECIAL_PUNCT(1),
    SPECIAL_PUNCT(2),
};
...
parser_set_encode(struct parser_params *p, const char *name)
{
    ...
    if (idx < 0) {
      ...
      error:
        ...
        ruby_show_error_line2(p, exc, &(YYLTYPE)RUBY_INIT_YYLLOC(), p->ruby_sourceline, p->lex.lastline);
...
parse_ident(struct parser_params *p, int c, int cmd_state)
{
    ...
    if (peek_end_expect_token_locations(p)) {
        ...
        if (p->debug) {
            rb_parser2_printf(p, "enforce_keyword_end check. current: (%d, %d), peek: (%d, %d)\n",
        ...
        if ((p->ruby_sourceline > lineno) && (beg_pos <= column)) {
            ...
            if ((IS_lex_state(EXPR_DOT)) && (kw = rb_reserved_word(tok(p), toklen(p))) && (kw && kw->id[0] == keyword_end)) {
                if (p->debug) rb_parser2_printf(p, "enforce_keyword_end is enabled\n");
...
{
    NODE *n = rb_ast2_newnode(p->ast, type, size, alignment);
...
rb_node_sym_new(struct parser_params *p, VALUE str, const YYLTYPE *loc)
{
    ...
    n->string = rb_str_to_parser2_string(p, str);
...
rb_node_file_new(struct parser_params *p, VALUE str, const YYLTYPE *loc)
{
    ...
    n->path = rb_str_to_parser2_string(p, str);
      ...
      case NODE_DSTR:
        if (htype == NODE_STR) {
            if (!literal_concat0(p, RNODE_STR(head)->string, RNODE_DSTR(tail)->string))
                goto error;
            rb_parser2_string_free(p, RNODE_DSTR(tail)->string);
...
{
    if (p->debug) {
        ls = rb_parser2_trace_lex_state(p, p->lex.state, ls, line);
    }
...
enum lex_state_e
rb_parser2_trace_lex_state(struct parser_params *p, enum lex_state_e from,
...
VALUE
rb_parser_lex_state_name2(struct parser_params *p, enum lex_state_e state)
{
...
void
rb_parser2_show_bitstack(struct parser_params *p, stack_type stack,
...
void
rb_parser2_fatal(struct parser_params *p, const char *fmt, ...)
{
    va_list ap;
...
YYLTYPE *
rb_parser2_set_location_from_strterm_heredoc(struct parser_params *p, rb_strterm_heredoc_t *here, YYLTYPE *yylloc)
{
...
YYLTYPE *
rb_parser2_set_location_of_delayed_token(struct parser_params *p, YYLTYPE *yylloc)
{
...
YYLTYPE *
rb_parser2_set_location_of_heredoc_end(struct parser_params *p, YYLTYPE *yylloc)
{
...
YYLTYPE *
rb_parser2_set_location_of_dummy_end(struct parser_params *p, YYLTYPE *yylloc)
{
...
YYLTYPE *
rb_parser2_set_location_of_none(struct parser_params *p, YYLTYPE *yylloc)
{
    int sourceline = p->ruby_sourceline;
    int beg_pos = (int)(p->lex.ptok - p->lex.pbeg);
...
YYLTYPE *
rb_parser2_set_location(struct parser_params *p, YYLTYPE *yylloc)
{
    int sourceline = p->ruby_sourceline;
    int beg_pos = (int)(p->lex.ptok - p->lex.pbeg);
...
warn_unused_var(struct parser_params *p, struct local_vars *local)
{
    ...
    if (cnt != local->vars->pos) {
        rb_parser2_fatal(p, "local->used->pos != local->vars->pos");
    }
#ifndef RIPPER
    ID *v = local->vars->tbl;
    ID *u = local->used->tbl;
...
local_tbl(struct parser_params *p)
{
    ...
    if (cnt <= 0) return 0;
    tbl = rb_ast2_new_local_table(p->ast, cnt);
    ...
    if (j < cnt) {
        tbl = rb_ast2_resize_latest_local_table(p->ast, j);
    }
...
#ifndef RIPPER
int
rb_parser2_local_defined(struct parser_params *p, ID id, const struct rb_iseq_struct *iseq)
{
...
local_id_ref(struct parser_params *p, ID id, ID **vidrefp)
{
    ...
    if (vars && vars->prev == DVARS_INHERIT) {
        return rb_parser2_local_defined(p, id, p->parent_iseq);
    }
...
forwarding_arg_check(struct parser_params *p, ID arg, ID all, const char *var)
{
    ...
    if (vars && vars->prev == DVARS_INHERIT && !found) {
        found = (rb_parser2_local_defined(p, arg, p->parent_iseq) &&
                 !(all && rb_parser2_local_defined(p, all, p->parent_iseq)));
    }
    else {
...
int
rb_reg_fragment_setenc2(struct parser_params* p, rb_parser_string_t *str, int options)
{
...
{
    int c = rb_reg_fragment_setenc2(p, str, options);
          ...
          int back_num, int *back_refs, OnigRegex regex, void *arg0)
{
    ...
    return rb_reg_named_capture_assign_iter_impl2(p, s, len, enc, &arg->succ_block, arg->loc, arg->assignable);
}
...
#ifndef RIPPER
NODE *
rb_parser_assignable2(struct parser_params *p, ID id, NODE *val, const YYLTYPE *loc)
{
    return assignable(p, id, val, loc);
}

int
rb_reg_named_capture_assign_iter_impl2(struct parser_params *p, const char *s, long len,
...
parser_reg_compile(struct parser_params* p, rb_parser_string_t *str, int options)
{
    ...
    return rb_parser_reg_compile2(p, str2, options);
}

#ifndef RIPPER
VALUE
rb_parser_reg_compile2(struct parser_params* p, VALUE str, int options)
{
...
#ifndef RIPPER
void
rb_ruby_parser2_set_options(struct parser_params *p, int print, int loop, int chomp, int split)
{
    p->do_print = print;
    p->do_loop = loop;
    p->do_chomp = chomp;
...
#ifdef RIPPER
#define rb_ruby_parser2_mark ripper_parser_mark
#define rb_ruby_parser2_free ripper_parser_free
#define rb_ruby_parser2_memsize ripper_parser_memsize
#endif

void
rb_ruby_parser2_mark(void *ptr)
{
...
size_t
rb_ruby_parser2_memsize(const void *ptr)
{
    struct parser_params *p = (struct parser_params*)ptr;
    struct local_vars *local;
...
rb_reserved_word2(const char *str, unsigned int len)
{
...
#ifdef UNIVERSAL_PARSER
rb_parser_t *
rb_ruby_parser2_allocate(const rb_parser_config_t *config)
{
...
rb_parser_t *
rb_ruby_parser2_new(const rb_parser_config_t *config)
{
    /* parser_initialize expects fields to be set to 0 */
    rb_parser_t *p = rb_ruby_parser2_allocate(config);
    parser_initialize(p);
    return p;
}
#else
rb_parser_t *
rb_ruby_parser2_allocate(void)
{
    /* parser_initialize expects fields to be set to 0 */
...
rb_parser_t *
rb_ruby_parser2_new(void)
{
    /* parser_initialize expects fields to be set to 0 */
    rb_parser_t *p = rb_ruby_parser2_allocate();
    parser_initialize(p);
    return p;
}
#endif
...
rb_parser_t *
rb_ruby_parser2_set_context(rb_parser_t *p, const struct rb_iseq_struct *base, int main)
{
...
void
rb_ruby_parser2_set_script_lines(rb_parser_t *p)
{
    p->debug_lines = rb_parser_ary_new_capa_for_script_line(p, 10);
}

void
rb_ruby_parser2_error_tolerant(rb_parser_t *p)
{
    p->error_tolerant = 1;
}

void
rb_ruby_parser2_keep_tokens(rb_parser_t *p)
{
    p->keep_tokens = 1;
...
rb_encoding *
rb_ruby_parser2_encoding(rb_parser_t *p)
{
    return p->enc;
}

int
rb_ruby_parser2_end_seen_p(rb_parser_t *p)
{
    return p->ruby__end__seen;
}

int
rb_ruby_parser2_set_yydebug(rb_parser_t *p, int flag)
{
    p->debug = flag;
    return flag;
}
...
#ifdef RIPPER
int
rb_ruby_parser2_get_yydebug(rb_parser_t *p)
{
    return p->debug;
}

void
rb_ruby_parser2_set_value(rb_parser_t *p, VALUE value)
{
    p->value = value;
}

int
rb_ruby_parser2_error_p(rb_parser_t *p)
{
    return p->error_p;
}

VALUE
rb_ruby_parser2_debug_output(rb_parser_t *p)
{
    return p->debug_output;
}

void
rb_ruby_parser2_set_debug_output(rb_parser_t *p, VALUE output)
{
    p->debug_output = output;
}

VALUE
rb_ruby_parser2_parsing_thread(rb_parser_t *p)
{
    return p->parsing_thread;
}

void
rb_ruby_parser2_set_parsing_thread(rb_parser_t *p, VALUE parsing_thread)
{
    p->parsing_thread = parsing_thread;
}

void
rb_ruby_parser2_ripper_initialize(rb_parser_t *p, rb_parser_lex_gets_func *gets, rb_parser_input_data input, VALUE sourcefile_string, const char *sourcefile, int sourceline)
{
    p->lex.gets = gets;
    p->lex.input = input;
    p->eofp = 0;
...
VALUE
rb_ruby_parser2_result(rb_parser_t *p)
{
    return p->result;
}

rb_encoding *
rb_ruby_parser2_enc(rb_parser_t *p)
{
    return p->enc;
}

VALUE
rb_ruby_parser2_ruby_sourcefile_string(rb_parser_t *p)
{
    return p->ruby_sourcefile_string;
}

int
rb_ruby_parser2_ruby_sourceline(rb_parser_t *p)
{
    return p->ruby_sourceline;
}

int
rb_ruby_parser2_lex_state(rb_parser_t *p)
{
    return p->lex.state;
}
...
{
    return rb_parser_lex_state_name2(p, (enum lex_state_e)state);
}
...
#ifndef RIPPER
void
rb_parser2_printf(struct parser_params *p, const char *fmt, ...)
{
    va_list ap;
    VALUE mesg = p->debug_buffer;
 ...
 */
size_t
rb_yytnamerr2(struct parser_params *p, char *yyres, const char *yystr)
{
    if (*yystr == '"') {
