...
class TestCoverage < Test::Unit::TestCase
  ...
  def test_branch_coverage_for_if_statement
    result = {
      :branches => {
        [:if    , 0, 2, 2] => {[:then, 1,  3]=>2, [:else, 2, 5]=>1},
        [:unless, 3, 8, 2] => {[:else, 4, 11]=>2, [:then, 5, 9]=>1},
      }
    }
  ...
  def test_branch_coverage_for_while_statement
    result = {
      :branches => {
        [:while, 0, 2, 0] => {[:body, 1, 3]=> 3},
        [:until, 2, 5, 0] => {[:body, 3, 6]=>10},
      }
    }
    assert_coverage(<<~"end;", { branches: true }, result)
      x = 3
  ...
  def test_branch_coverage_for_case_statement
    result = {
      :branches => {
        [:case,  0,  2, 2] => {[:when,  1,  4]=>2, [:when,  2,  6]=>0, [:else,  3,  2]=>1},
        [:case,  4,  9, 2] => {[:when,  5, 11]=>2, [:when,  6, 13]=>0, [:else,  7,  9]=>1},
        [:case,  8, 16, 2] => {[:when,  9, 18]=>2, [:when, 10, 20]=>0, [:else, 11, 22]=>1},
        [:case, 12, 25, 2] => {[:when, 13, 27]=>2, [:when, 14, 29]=>0, [:else, 15, 31]=>1},
      }
    }
    assert_coverage(<<~"end;", { branches: true }, result)
      def foo(x)
  ...
  def test_branch_coverage_for_safe_method_invocation
    result = {
      :branches=>{
        [:"&.", 0, 3, 0] => {[:then, 1, 3]=>1, [:else, 2, 3]=>0},
        [:"&.", 3, 4, 0] => {[:then, 4, 4]=>0, [:else, 5, 4]=>1},
      }
    }
    assert_coverage(<<~"end;", { branches: true }, result)
      a = 10
