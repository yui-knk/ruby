...
endless_command : command
                ...
                | keyword_not opt_nl endless_command
                    ...
                    /*% ripper: unary!(STATIC_ID2SYM(idNOT), $:3) %*/
                    }
                ;
...
expr		: command_call
                | expr keyword_and expr
                    {
                        $$ = logop(p, idAND, $1, $3, &@2, &@$);
                    /*% ripper: binary!($:1, STATIC_ID2SYM(idAND), $:3) %*/
                    }
...
arg		: lhs '=' lex_ctxt arg_rhs
                ...
                | arg '+' arg
                    {
                        $$ = call_bin_op(p, $1, '+', $3, &@2, &@$);
                    /*% ripper: binary!($:1, STATIC_ID2SYM('\'+\''), $:3) %*/
                    }
                | arg '-' arg
                    {
                        $$ = call_bin_op(p, $1, '-', $3, &@2, &@$);
                ...
                | arg tPOW arg
                    ...
                    /*% ripper: binary!($:1, STATIC_ID2SYM(idPow), $:3) %*/
                    }
                | tUMINUS_NUM simple_numeric tPOW arg
                    ...
                    /*%%%*/
                    /*%
                        VALUE val = dispatch3(binary, get_value($:2), STATIC_ID2SYM(idPow), get_value($:4));
                        val = dispatch2(unary, STATIC_ID2SYM(idUMinus), val);
                        set_value(val);
                    %*/
                    }
                | tUPLUS arg
                    ...
                    /*% ripper: unary!(STATIC_ID2SYM(idUPlus), $:2) %*/
                    }
                | tUMINUS arg
                    {
                ...
                | arg tCMP arg
                    ...
                    /*% ripper: binary!($:1, STATIC_ID2SYM(idCmp), $:3) %*/
                    }
                | rel_expr   %prec tCMP
                | arg tEQ arg
                    {
                        $$ = call_bin_op(p, $1, idEq, $3, &@2, &@$);
                    /*% ripper: binary!($:1, STATIC_ID2SYM(idEq), $:3) %*/
                    }
                | arg tEQQ arg
                    {
                        $$ = call_bin_op(p, $1, idEqq, $3, &@2, &@$);
                    /*% ripper: binary!($:1, STATIC_ID2SYM(idEqq), $:3) %*/
                    }
                | arg tNEQ arg
                    {
                        $$ = call_bin_op(p, $1, idNeq, $3, &@2, &@$);
                    /*% ripper: binary!($:1, STATIC_ID2SYM(idNeq), $:3) %*/
                    }
                | arg tMATCH arg
                    {
                        $$ = match_op(p, $1, $3, &@2, &@$);
                    /*% ripper: binary!($:1, STATIC_ID2SYM(idEqTilde), $:3) %*/
                    }
                | arg tNMATCH arg
                    {
                        $$ = call_bin_op(p, $1, idNeqTilde, $3, &@2, &@$);
                    /*% ripper: binary!($:1, STATIC_ID2SYM(idNeqTilde), $:3) %*/
                    }
                | '!' arg
                    {
                        $$ = call_uni_op(p, method_cond(p, $2, &@2), '!', &@1, &@$);
                ...
                | arg tRSHFT arg
                    ...
                    /*% ripper: binary!($:1, STATIC_ID2SYM(idGTGT), $:3) %*/
                    }
                | arg tANDOP arg
                    {
                        $$ = logop(p, idANDOP, $1, $3, &@2, &@$);
                    /*% ripper: binary!($:1, STATIC_ID2SYM(idANDOP), $:3) %*/
                    }
                | arg tOROP arg
                    {
                        $$ = logop(p, idOROP, $1, $3, &@2, &@$);
                    /*% ripper: binary!($:1, STATIC_ID2SYM(idOROP), $:3) %*/
                    }
...
rel_expr	: arg relop arg   %prec '>'
                    ...
                    /*% ripper: binary!($:1, STATIC_ID2SYM($2), $:3) %*/
                    }
...
primary		: literal
                ...
                | keyword_not '(' rparen
                    ...
                    /*% ripper: unary!(STATIC_ID2SYM(idNOT), Qnil) %*/
                    }
                | fcall brace_block
                    {
...
#else
/* TODO: Fix RB_OBJ_WRITTEN, p->ast is wrong. */
#define set_parser_s_value(x) (p->s_value = (x), RB_OBJ_WRITTEN(p->ast, Qnil, p->s_value))
#endif
