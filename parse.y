...
static void
debug_parser_string0(struct parser_params *p, const char *header, rb_parser_string_t *str, bool newline, int line)
{
    if (!p->debug) return;
    ...
    if (str)
        fprintf(stderr, " %p (%p - %p) %6ld: %s\n", str, str->ptr, &str->ptr[str->len], str->len, str->ptr);
    else
        fprintf(stderr, "(NULL)\n");
...
static void
debug_parser_params(struct parser_params *p, const char *header)
{
    if (!p->debug) return;
    ...
    if (p->lex.lastline)
        fprintf(stderr, "(%p): %s\n", p->lex.lastline->ptr, p->lex.lastline->ptr);
    else
        fprintf(stderr, "(NULL)\n");

    fprintf(stderr, "    > nextline");
    if (p->lex.nextline > (rb_parser_string_t *)1)
        fprintf(stderr, "(%p): %s\n", p->lex.nextline->ptr, p->lex.nextline->ptr);
...
#define debug_parser_params(p, h) ((void)0)
#endif
...
static void
rb_parser_string_free0(rb_parser_t *p, rb_parser_string_t *str)
{
    debug_parser_string("free    ", str, false);
    xfree(str);
}

// #define rb_parser_string_free(p, str) (fprintf(stderr, "%d\n", __LINE__), rb_parser_string_free0(p, str))
#define rb_parser_string_free(p, str) (rb_parser_string_free0(p, str))
...
nextline(struct parser_params *p, int set_encoding)
{
    ...
    debug_parser_params(p, "nextline");
    ...
    p->ruby_sourceline++;
    if (p->lex.free_lastline) {
        rb_parser_string_free(p, p->lex.lastline);
    }
    p->lex.free_lastline = false;
    set_lastline(p, str);
    token_flush(p);
    return 0;
}
...
yylex(YYSTYPE *lval, YYLTYPE *yylloc, struct parser_params *p)
{
    ...
    lval->val = Qundef;
    p->yylloc = yylloc;
    debug_parser_params(p, "parser_yylex start =========");
    t = parser_yylex(p);
    debug_parser_params(p, "parser_yylex end   =========");
...
rb_ruby_parser_free(void *ptr)
{
    ...
    debug_parser_string("rb_ruby_parser_free nextline", p->lex.nextline, false);
    debug_parser_string("rb_ruby_parser_free lastline", p->lex.lastline, false);
    if (p->tokenbuf) {
