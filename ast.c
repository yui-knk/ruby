...
static VALUE
var_name(rb_parser_string_t *id)
{
    if (!id) return Qnil;
    // if (!rb_id2str(id)) return Qnil;
    return rb_parser_string_to_sym(id);
}
    ...
    switch (type) {
      ...
      case NODE_CDECL:
        if (RNODE_CDECL(node)->nd_vid) {
            return rb_ary_new_from_args(2, rb_parser_string_to_sym(RNODE_CDECL(node)->nd_vid), NEW_CHILD(ast, RNODE_CDECL(node)->nd_value));
        }
        return rb_ary_new_from_args(3, NEW_CHILD(ast, RNODE_CDECL(node)->nd_else), rb_parser_string_to_sym(RNODE_COLON2(RNODE_CDECL(node)->nd_else)->nd_mid), NEW_CHILD(ast, RNODE_CDECL(node)->nd_value));
      case NODE_OP_ASGN1:
        return rb_ary_new_from_args(4, NEW_CHILD(ast, RNODE_OP_ASGN1(node)->nd_recv),
                                    rb_parser_string_to_sym(RNODE_OP_ASGN1(node)->nd_mid),
      ...
      case NODE_OP_ASGN2:
        return rb_ary_new_from_args(5, NEW_CHILD(ast, RNODE_OP_ASGN2(node)->nd_recv),
                                    ...
                                    rb_parser_string_to_sym(RNODE_OP_ASGN2(node)->nd_vid),
                                    rb_parser_string_to_sym(RNODE_OP_ASGN2(node)->nd_mid),
      ...
      case NODE_IVAR:
        return rb_ary_new_from_args(1, rb_parser_string_to_sym(RNODE_IVAR(node)->nd_vid));
      case NODE_CONST:
        return rb_ary_new_from_args(1, rb_parser_string_to_sym(RNODE_CONST(node)->nd_vid));
      case NODE_CVAR:
        return rb_ary_new_from_args(1, rb_parser_string_to_sym(RNODE_CVAR(node)->nd_vid));
      case NODE_GVAR:
        return rb_ary_new_from_args(1, rb_parser_string_to_sym(RNODE_GVAR(node)->nd_vid));
      case NODE_NTH_REF:
      ...
      case NODE_DEFN:
        return rb_ary_new_from_args(2, rb_parser_string_to_sym(RNODE_DEFN(node)->nd_mid), NEW_CHILD(ast, RNODE_DEFN(node)->nd_defn));
      case NODE_DEFS:
        return rb_ary_new_from_args(3, NEW_CHILD(ast, RNODE_DEFS(node)->nd_recv), rb_parser_string_to_sym(RNODE_DEFS(node)->nd_mid), NEW_CHILD(ast, RNODE_DEFS(node)->nd_defn));
      case NODE_ALIAS:
        return rb_ary_new_from_node_args(ast, 2, RNODE_ALIAS(node)->nd_1st, RNODE_ALIAS(node)->nd_2nd);
      case NODE_VALIAS:
        return rb_ary_new_from_args(2, rb_str_new_parser_string(RNODE_VALIAS(node)->nd_alias), rb_str_new_parser_string(RNODE_VALIAS(node)->nd_orig));
      case NODE_UNDEF:
      ...
      case NODE_COLON2:
        return rb_ary_new_from_args(2, NEW_CHILD(ast, RNODE_COLON2(node)->nd_head), rb_parser_string_to_sym(RNODE_COLON2(node)->nd_mid));
      case NODE_COLON3:
        return rb_ary_new_from_args(1, rb_parser_string_to_sym(RNODE_COLON3(node)->nd_mid));
      case NODE_DOT2:
      case NODE_DOT3:
      case NODE_FLIP2:
      case NODE_FLIP3:
      ...
      case NODE_ATTRASGN:
        return rb_ary_new_from_args(3, NEW_CHILD(ast, RNODE_ATTRASGN(node)->nd_recv), rb_parser_string_to_sym(RNODE_ATTRASGN(node)->nd_mid), NEW_CHILD(ast, RNODE_ATTRASGN(node)->nd_args));
      case NODE_LAMBDA:
