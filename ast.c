...
rb_ary_new_from_node_args(long n, ...)
{
	...
	NODE *node;
	node = va_arg(ar, NODE *);
	rb_ary_push(ary, node ? ast_new_child_internal(node) : Qnil);
    }
    va_end(ar);
    return ary;
}
...
node_children(NODE *node)
{
    switch (nd_type(node)) {
      case NODE_BLOCK:
	return rb_ary_new_from_node_args(2, node->nd_head, node->nd_next);
      case NODE_IF:
	...
	return rb_ary_new_from_node_args(1, node->nd_value);
      case NODE_GASGN:
	return rb_ary_new_from_node_args(2, node->nd_entry, node->nd_value);
      case NODE_CDECL:
        if (node->nd_vid) {
	    return rb_ary_new_from_node_args(1, node->nd_value);
        }
	...
	return rb_ary_new_from_node_args(1, node->nd_defn);
      case NODE_DEFS:
	return rb_ary_new_from_node_args(2, node->u1.node, node->u2.node);
      case NODE_VALIAS:
	return rb_ary_new_from_node_args(0);
      case NODE_UNDEF:
	return rb_ary_new_from_node_args(1, node->u2.node);
      case NODE_CLASS:
...
rb_ast_inspect(VALUE self)
{
    ...
    rb_str_cat2(str, "(");
    rb_str_catf(str, "%s %d:%d, %d:%d", node_type_to_str(data->node), nd_lineno(data->node), nd_column(data->node), nd_last_lineno(data->node), nd_last_column(data->node));
    rb_str_cat2(str, "): >");
